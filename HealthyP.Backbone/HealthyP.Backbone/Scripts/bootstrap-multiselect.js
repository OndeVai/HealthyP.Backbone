...
/**
 * Bootstrap multiselect data api (custom by company)
 *  
 * This is a fix to the bootstrap multiselect plugin to allow data- attributes that map to script options
 * In addition some specific data is applied (data-hack-selected, etc)
 * In addition some reasonable defaults are set (enableCaseInsensitiveFiltering=true, etc)
 */
!function ($) {
    ...
    var origMultiselect = $.fn.multiselect;
    $.fn.multiselect = function (option, parameter) {

        //todo patch the non "data-" attributes
        
        //todo add the custom logic (filter > 10, etc, data hack selected)
        var hackSelected = this.data('hack-selected');
        if (hackSelected) {
            this.val(hackSelected);
        }

        if (!option) {
            option = opts_from_el(this, prefix);
            option = $.extend({}, option, defaults);

        }

        return origMultiselect.call(this, option, parameter);
    };
